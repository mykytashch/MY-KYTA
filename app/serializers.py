...
from .models import User, Panel, Data, PostRequest


from rest_framework import serializers

class JsonOrderSerializer(serializers.Serializer):
    customer_name = serializers.CharField(max_length=255)  # Имя покупателя.
    product = serializers.CharField(max_length=255)  # Название продукта.
    created_at = serializers.DateTimeField(read_only=True)  # Дата создания (только для чтения).
    updated_at = serializers.DateTimeField(read_only=True)  # Дата обновления (только для чтения).
